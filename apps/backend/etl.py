...
async def sync_movies(db: Session, tmdb_api_key: str):
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        existing_set = {
            f"{m.title}_{m.director}_{m.year}".lower().strip()
        ...
        for i, item in enumerate(movies_to_process):
            ...
            if tmdb_api_key:
                params = {"query": item["title"], "language": "es-AR", "api_key": tmdb_api_key}
                ...
                try:
                    ...
                    if results:
                        ...
                        details_res = await client.get(f"{TMDB_DETAILS_URL}{movie_id}", params={"language": "es-AR", "api_key": tmdb_api_key})
